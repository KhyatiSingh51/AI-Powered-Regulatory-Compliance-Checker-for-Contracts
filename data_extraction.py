...
def Clause_extraction(file):
    """
    Extract clauses from PDF using Gemini API.
    """
    print("Inside Clause_extraction")
...
def Clause_extraction_with_summarization(file):
    """
    Extract clauses and summarise text using Gemini API.
    Raises Exception with 'MODEL_OVERLOAD' if model is overloaded.
    """
    print("Inside Clause_extraction_with_summarization")
    ...
    Extract clauses from the following contract and summarise each clause:
    ...
    try:
        ...
        # Raise custom overload exception
        if "overload" in str(e).lower():
            raise Exception("MODEL_OVERLOAD: Gemini API overloaded")
        else:
            raise e


def Clause_extraction_with_summarization_groq(file):
    """
    Extract clauses with summarisation using Groq accelerated inference.
    """
    print("Using Groq accelerated clause extraction...")
    ...
    prompt = f"""
    You are an expert legal contract analyst.
    Extract clauses with summarised text from the following contract:
    ...
    response = groq_client.run_model(
        model_name="contract_clause_model",
        input_text=prompt
    )
